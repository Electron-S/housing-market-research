...
리뷰 패킷을 읽고 **보고서 유형에 맞는 기준 표를 선택해** 5개 기준을 각각 A/B/C로 평가한다.
...
| 번호 | 평가항목 | 고평가 기준 |
|---|---|---|
| 1 | 데이터 신뢰성 | 임대료·공실·매매가 데이터에 출처와 기준 시점이 명시됐는가 |
| 2 | 시장 해석 | 상권 흐름·수요층·업종 구성이 분양성과 연결되어 설명됐는가 |
| 3 | 입지 분석 | 가시성·동선·층별 집객력이 배후 수요층과 구체적으로 연결됐는가 |
| 4 | 리스크 반영 | 공실 장기화·업종 편중·동선 분리 등 상가 고유 리스크가 구조화됐는가 |
| 5 | 결론 실효성 | 층별 적정 평단가 밴드(1층 핵심/비핵심, 비1층 목적형/일반형)가 수치로 제시됐는가 |

**상가 전용 추가 확인:** STEP9의 층별 평단가 밴드 표가 보고서 본문에 실제 반영됐는지 검증한다.
(보조 3종 보고서는 아파트 표를 기준으로 평가하되, 해당 없는 항목은 산출물 성격에 맞게 해석한다.)
