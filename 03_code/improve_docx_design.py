...
사용법:
  ...
  python improve_docx_design.py in.docx out.docx --no-final   # 05_output 배치 생략
  python improve_docx_design.py in.docx out.docx --final-dir [폴더]  # 배치 폴더 변경

종료코드: 0=성공, 1=실행 실패, 2=일부 항목 실패(디자인 부분 적용 등)
"""
...
from _common import (
    ...
    WORKSPACE_DIR,
    configure_stdout,
    extract_agent_tag_from_filename,
...
def build_final_output_path(target_id, designed_path, final_dir=None):
    """최종본을 05_output(또는 지정 폴더) 아래에 배치할 경로를 생성

    파일명 정본: [작업폴더명]_designed.docx
    """
    target_id = (target_id or "").strip()
    final_dir = Path(final_dir) if final_dir else OUTPUT_DIR
...
def improve_heading_styles(doc, issues):
    """제목 스타일을 개선. 실패 항목은 issues에 수집한다."""
    print("  제목 스타일 개선 중...")
    ...
    try:
        ...
        set_east_asia_font(h1_style, FONTS['heading'])
        ...
        issues.append(f"Heading 1 스타일 설정 오류: {e}")
    ...
    try:
        ...
        set_east_asia_font(h2_style, FONTS['heading'])

        h2_style.paragraph_format.space_before = Pt(12)
        h2_style.paragraph_format.space_after = Pt(4)
        h2_style.paragraph_format.keep_with_next = True
    except Exception as e:
        issues.append(f"Heading 2 스타일 설정 오류: {e}")
    ...
    try:
        ...
        set_east_asia_font(normal_style, FONTS['body'])

        normal_style.paragraph_format.line_spacing = 1.15
        normal_style.paragraph_format.space_after = Pt(6)
    except Exception as e:
        issues.append(f"Normal 스타일 설정 오류: {e}")
        print(f"    Normal 스타일 설정 오류: {e}")

def improve_tables(doc, issues):
    """표의 디자인을 개선. 실패한 표는 issues에 수집한다."""
    print("  표 디자인 개선 중...")
        ...
        try:
            ...
            if table.rows:
                ...
                for cell in header_row.cells:
                    ...
                    # 테두리선
                    set_cell_borders(cell, '003366', '8', clear_existing=True)
                    # 텍스트 스타일
            ...
            for row_idx, row in enumerate(table.rows[1:], 1):
                for cell in row.cells:
                    ...
                    # 테두리선
                    set_cell_borders(cell, 'B4B4B4', '4', clear_existing=True)
                    # 텍스트 스타일
                    for para in cell.paragraphs:
                        for run in para.runs:
                            run.font.size = Pt(9)
            ...
            issues.append(f"표{table_idx+1} 스타일 설정 오류: {e}")
...
def add_header_footer(doc, target_id, company_name=""):
    ...
    for section_idx, section in enumerate(doc.sections):
        ...
        set_run_font(run, FONTS['heading'], size_pt=8, color=COLORS['secondary'])
        ...
        set_run_font(run1, FONTS['body'], size_pt=9, color=COLORS['secondary'])
        add_page_number(run1)

        run2 = footer_para.add_run(" / ")
        set_run_font(run2, FONTS['body'], size_pt=9, color=COLORS['secondary'])

        run3 = footer_para.add_run()
        set_run_font(run3, FONTS['body'], size_pt=9, color=COLORS['secondary'])
...
def improve_cover_page(doc, target_id):
    ...
    for i, para in cover_paragraphs:
        ...
        if '부동산 분석 보고서' in text:
            for run in para.runs:
                set_run_font(run, FONTS['title'], size_pt=28, bold=True, color=COLORS['primary'])
        ...
        # 분석 대상명
            ...
            for run in para.runs:
                set_run_font(run, FONTS['title'], size_pt=16, bold=True, color=COLORS['primary'])
            para.paragraph_format.space_after = Pt(2)

        # 분석대상 ID
        elif target_id and target_id in text:
            for run in para.runs:
                set_run_font(run, FONTS['body'], size_pt=11, color=COLORS['secondary'])

        # 작성자·날짜(하단 메타 정보)
        elif '작성자' in text or ('년' in text and len(text) < 20):
            for run in para.runs:
                set_run_font(run, FONTS['body'], size_pt=10, color=COLORS['secondary'])

        # 서브 타이틀(제안서 제목 등, 위에 해당하지 않는 긴 텍스트)
        else:
            for run in para.runs:
                set_run_font(run, FONTS['title'], size_pt=13, italic=True, color=COLORS['secondary'])
        ...
        # 일반 본문
            ...
            for run in para.runs:
                if run.font.size is None:
                    run.font.size = Pt(10)
                set_run_font(run, FONTS['body'])

def process_document(input_path, output_path=None, final_dir=None, place_final=True):
    """문서 전체를 처리. 실패 항목 리스트를 반환한다 (빈 리스트 = 완전 성공)."""
    print(f"\n처리 중: {input_path}")
    ...
    # 분석대상 ID/이름 추출
    target_id = build_header_label(extract_target_id_from_docx(input_path))
    company_name = extract_company_name(doc)
    print(f"  분석대상 ID: {target_id}")
    print(f"  분석대상명: {company_name if company_name else '(검출 실패)'}")

    # 디자인 개선을 적용 (실패 항목은 issues에 수집)
    issues = []
    improve_heading_styles(doc, issues)
    improve_tables(doc, issues)
    ...
    if place_final:
        final_output_path = build_final_output_path(target_id, output_path, final_dir=final_dir)
        shutil.copy2(output_path, final_output_path)
        print(f"  최종본 배치 완료: {final_output_path}")

    if issues:
        print(f"  주의: {len(issues)}개 항목이 적용되지 않음:")
        for issue in issues:
            print(f"    - {issue}")

    return issues

def process_all_documents(final_dir=None, place_final=True):
    """04_workspace 하위의 draft DOCX를 일괄 처리. 실패 파일 리스트를 반환한다."""
    ...
    for input_path in docx_files:
        ...
        try:
            issues = process_document(str(input_path), str(output_path),
                                      final_dir=final_dir, place_final=place_final)
            if issues:
                failed.append((input_path, f"{len(issues)}개 항목 미적용"))
        except Exception as e:
            failed.append((input_path, str(e)))
            print(f"  오류 ({input_path}): {e}")

    print(f"\n완료! 작업본 경로: {WORKSPACE_DIR}")
    print(f"최종본 경로: {final_dir or OUTPUT_DIR}")
    if failed:
        print(f"실패/부분 적용 {len(failed)}건:")
...
def main():
    configure_stdout()
    parser = argparse.ArgumentParser(description="DOCX 디자인 보정")
    parser.add_argument("input", nargs="?", help="입력 DOCX 경로")
    parser.add_argument("output", nargs="?", help="출력 DOCX 경로 (기본: 입력명 기반 자동)")
    parser.add_argument("--all", action="store_true", help="04_workspace 하위 draft 전체를 일괄 처리")
    parser.add_argument("--final-dir", help="최종본 배치 폴더 (기본: 05_output)")
    parser.add_argument("--no-final", action="store_true", help="최종본 배치(05_output 복사)를 생략")
    args = parser.parse_args()

    if args.all == bool(args.input):
        parser.error("입력파일 또는 --all 중 하나를 지정한다.")

    place_final = not args.no_final

    if args.all:
        failed = process_all_documents(final_dir=args.final_dir, place_final=place_final)
        sys.exit(EXIT_PARTIAL if failed else EXIT_OK)
    ...
    try:
        issues = process_document(args.input, args.output,
                                  final_dir=args.final_dir, place_final=place_final)
    except Exception as e:
        print(f"ERROR: 처리 실패 ({args.input}): {e}", file=sys.stderr)
        sys.exit(EXIT_ERROR)

    sys.exit(EXIT_PARTIAL if issues else EXIT_OK)
